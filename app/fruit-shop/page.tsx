...
import FruitShopGameScreen from '@/components/FruitShopGameScreen';
import FruitShopSettings from '@/components/FruitShopSettings';
import FruitShopGameOver from '@/components/FruitShopGameOver';
import Link from 'next/link';

export default function FruitShopPage() {
  const { isPlaying, score, stats, startGame, resetGame, initFruitShop, questionIndex } = useFruitShopStore();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-100 via-yellow-100 to-green-100 flex items-center justify-center p-4">
      <div className="max-w-2xl w-full">
        ...
        >
          ...
          <motion.div
            ...
            className="mb-8"
          >
            <h1 className="text-6xl md:text-8xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-orange-600 to-green-600 mb-4">
              Fruit Shop
            </h1>
            <h2 className="text-4xl md:text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-yellow-600 to-orange-600">
              Money Math! 🍎💰
            </h2>
          </motion.div>
          ...
          >
            Count coins and pay for delicious fruit!
          </motion.p>
          ...
          {stats.bestScore > 0 && (
            ...
            >
              <div className="grid grid-cols-3 gap-4 text-center">
                ...
                <div>
                  <p className="text-3xl font-bold text-green-600">{stats.gamesPlayed}</p>
          ...
          >
            <button
              ...
              className="w-full px-8 py-6 bg-gradient-to-r from-orange-500 to-green-500 hover:from-orange-600 hover:to-green-600 text-white rounded-2xl font-bold text-2xl transition-all transform hover:scale-105 shadow-lg focus:outline-none focus:ring-4 focus:ring-orange-300"
            >
              🎮 Start Shopping
            </button>
      ...
      {/* Modals */}
      <AnimatePresence>
        {showSettings && <FruitShopSettings onClose={() => setShowSettings(false)} />}
        {showGameOver && (
          <FruitShopGameOver
            score={score}
